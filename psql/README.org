...
- [[#introduction-into-postgresql][Introduction into PostgreSQL]]
  - [[#selecting][Selecting]]
    - [[#regular-selection][Regular Selection]]
    - [[#select-distinct-rows-using-distinct-operator][Select distinct rows using ~distinct~ operator]]
    - [[#sort-rows-using-order-by][Sort rows using ~order by~]]
    - [[#filter-rows-using-where][Filter rows using ~where~]]
    - [[#in][IN]]
    - [[#group-using-group-by][Group using ~group by~]]
    - [[#filter-group-using-having][Filter group using ~having~]]
    - [[#perform-set-operations-using-union-intersect-and-except][Perform set operations using ~UNION~, ~intersect~ and ~except~]]
